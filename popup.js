...
function generatePassword(length) {
  ...
  if (excludeSimilar.checked) {
    chars = chars.replace(/[iIl1Lo0O5Ss2Z6b8B9gCc]/g, '');
  }
...
const excludeSimilarValue = localStorage.getItem('excludeSimilar');
...
if (!excludeSimilarValue) {
  localStorage.setItem('excludeSimilar', true);
  excludeSimilar.checked = true;
} else {
  excludeSimilar.checked = excludeSimilarValue === 'true';
}

//find if localstorage has passLen value
const passLenValue = localStorage.getItem('passLen');
if (!passLenValue) {
  localStorage.setItem('passLen', 16);
  passLen.value = 16;
...
excludeSimilar.addEventListener('change', () => {
  localStorage.setItem('excludeSimilar', excludeSimilar.checked);
});
